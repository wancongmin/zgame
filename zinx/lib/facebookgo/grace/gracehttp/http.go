...
const (
	SIGUSER2 = syscall.Signal(12)
)

func (a *app) signalHandler(wg *sync.WaitGroup) {
	ch := make(chan os.Signal, 10)
	signal.Notify(ch, syscall.SIGINT, syscall.SIGTERM, SIGUSER2)
	for {
		sig := <-ch
		switch sig {
...
func (a *app) run() error {
	...
	if didInherit && ppid != 1 {
		cmd := exec.Command("kill", "-SIGTERM", fmt.Sprintf("%d", ppid))
