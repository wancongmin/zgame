...
var WorkerPoolSize int = config.GetIntConfig("WorkerPoolSize",10)
var Requestcount int = config.GetIntConfig("Requestcount",1040)
func NewMsgHandle() *MsgHandle {
	return &MsgHandle{
		Apis:make(map[uint32]ziface.IRouter),
		WorkerPoolSize:uint32(WorkerPoolSize),                    //参数配置
		TaskQueue:make([]chan ziface.IRequest,Requestcount), //参数配置
	}
}
